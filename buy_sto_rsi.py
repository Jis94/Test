    ...
    try:
        ...
        while True:
            ...
            for target_item in target_items:
                ...
                #         and Decimal(str(rsi[2]['RSI'])) < Decimal(str(70))):
                #     rsi_val = True
                ...
                #         # and Decimal(str(rsi[3]['RSI'])) > Decimal(str(rsi[2]['RSI']))
                #         and Decimal(str(mfi[2]['MFI'])) < Decimal(str(70))):
                #     mfi_val = True
                ...
                if stochrsi_K.iloc[-1]*100 > 33 and stochrsi_D.iloc[-1]*100 <27 and rsi[0]['RSI'] < 40 and mfi[0]['MFI'] < 30:
                ...
                # if rsi_val:
                    ...
                    price[1]['low_price'] # 30분 전 저가
                    price[0]['low_price'] # 현재 저가
                    price[0]['trade_price'] # 현재가

                    price[3]['high_price'] # 90분 전 고가
                    ...
                    bb_data[2]['BBL'] # 60분 전 BBL
                    bb_data[1]['BBL'] # 30분 전 BBL
                    bb_data[0]['BBL'] # 현재 BBL

                    bb_data[3]['BBM'] # 90분 전 BBM
                    bb_data[2]['BBM'] # 60분 전 BBM
                    bb_data[1]['BBM'] # 30분 전 BBM
                    bb_data[0]['BBM'] # 현재 BBM

                    # bb_data[3]['BBH'] # 90분 전 BBH
                    # bb_data[2]['BBH'] # 60분 전 BBH
                    # bb_data[1]['BBH'] # 30분 전 BBH
                    # bb_data[0]['BBH'] # 현재 BBH

                    # 볼린저밴드 조회 (1일)
                    bb_data_d = get_bb(target_item['market'], 'D', '200', '4')
                    # for bb in bb_data:
                    # logging.info(bb_data)
                    bb_data_d[3]['BBL'] # 3일 전 BBL
                    bb_data_d[2]['BBL'] # 2일 전 BBL
                    bb_data_d[1]['BBL'] # 1일 전 BBL
                    bb_data_d[0]['BBL'] # 현재 BBL

                    bb_data_d[1]['BBM'] # 1일 전 BBL
                    bb_data_d[0]['BBM'] # 현재 BBM

                    bb_data_d[1]['BBH'] # 1일 전 BBH
                    bb_data_d[0]['BBH'] # 현재 BBH


                    # 가격 조회 (1일)
                    price_d = get_candle(target_item['market'], 'D', '4')
                    # for target_price in price:
                    price_d[3]['low_price'] # 3일 전 저가
                    price_d[2]['low_price'] # 2일 전 저가
                    price_d[1]['low_price'] # 전날 저가
                    price_d[0]['low_price'] # 오늘 저가
                

                    # ------------------------------------------------------------------
                    # 기매수 여부 판단
                    # ------------------------------------------------------------------
                    accounts = get_accounts('Y', 'KRW')
                    ...
                    # and (Decimal(str(price[0]['trade_price']))) > (Decimal(str(bb_data[0]['BBL'])))                    and (((Decimal(str(price[3]['low_price'])))+(Decimal(str(price[3]['high_price']))))/2) < (Decimal(str(bb_data[3]['BBL'])))
                ...
                # else:
                #     logging.info('------------------------------------------------------')
                #     logging.info('-----------볼린저 밴드 하단 돌파 조건 미충족----------')
                #     logging.info('----------------------재탐색 실시...------------------')
                #     logging.info('------------------------------------------------------')
